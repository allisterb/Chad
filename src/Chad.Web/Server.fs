...
module Server =        
    ...
    let getUser(user:string) : Async<User option> = 
        pgdb
        |> Sql.query "SELECT * FROM Chad_user WHERE user_name=@u"
    ...
    let addUser (user:string) : Async<Result<unit, string>> =
        pgdb
        |> Sql.query "INSERT INTO public.Chad_user(user_name, last_logged_in) VALUES (@u, @d);"
    ...
    let updateUserLastLogin (user:string) : Async<Result<unit, exn>> =
        pgdb
        |> Sql.query "UPDATE public.Chad_user SET last_logged_in=@d WHERE user_name=@u;"
        |> Sql.parameters [("u", Sql.string user); ("d", Sql.timestamp (DateTime.Now))]
        |> Sql.executeNonQueryAsync
        |> Async.map (
            function 
