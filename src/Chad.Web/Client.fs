...
module Client =
    ...
    let Main =             
        ...
        let mainOpt =
            Options(
                Name="Main", 
                Greetings = "Welcome to Chad. Enter 'hello' or 'hello my name is...(you) to initialize speech.",
                Prompt = "&#x2328;  " //&#x1f4ac;
            )       
    ...
    let run() =        
        ...
        CUI.EchoDoc <| Doc.Concat [
            Bs.btnPrimary "hello" (fun _ _ -> initMic(main');initSpeech();quick_utter "greet" |> push |> Chad.Web.Main.update)
            Html.text "     "
            Bs.btnInfo "help" (fun _ _ -> initMic(main');initSpeech();quick_utter "help" |> push |> Chad.Web.Main.update)
        ]
        Doc.Empty
